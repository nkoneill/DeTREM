...
DeTREM = function(bulk.eset, sc.eset, markers = NULL, clusters, samples, select.ct = NULL, cell_size = NULL, ct.cov = FALSE, verbose = TRUE,
  ...
  if(verbose){message(paste('Used', length(cm.gene), 'common genes...'))}
	scmean=rowSums(sc.eset@assayData$exprs)
	scmean=scmean*10000/sum(scmean)

	bulkmean=rowSums(bulk.eset@assayData$exprs)
	bulkmean=bulkmean*10000/sum(bulkmean)

	bulkmean=bulkmean[cm.gene]
	scmean=scmean[cm.gene]
	weights=bulkmean/scmean
	weights[is.na(weights)]=Inf
	weights=abs(log2(weights/median(weights)))*10
	weights[weights<1]=1
	rm(bulkmean,scmean)
  ...
  if(ct.cov){
    ...
    for(i in 1:N.bulk){
      ...
      lm.D1.weighted = music.iter(Yjg.temp, D1.temp, M.S, Sigma.temp, iter.max = iter.max,
                                  nu = nu, eps = eps, centered = centered, normalize = normalize,weights=weights)
      Est.prop.allgene = rbind(Est.prop.allgene, lm.D1.weighted$p.nnls)
      Est.prop.weighted = rbind(Est.prop.weighted, lm.D1.weighted$p.weight)
      weight.gene.temp = rep(NA, nrow(Yjg)); weight.gene.temp[Yjg[,i]!=0] = lm.D1.weighted$weight.gene;
      Weight.gene = cbind(Weight.gene, weight.gene.temp)
