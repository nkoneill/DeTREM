...
music.basic = function(Y, X, S, Sigma, iter.max, nu, eps,weights){
  k = ncol(X)

  lm.D = nnls(X, Y)
  r = resid(lm.D);
  weight.gene = 1/(nu + r^2 + colSums( (lm.D$x*S)^2*t(Sigma)  ))
		weight.gene=weight.gene*(1/weights[rownames(weight.gene)])
...
#' @export
music.iter = function(Y, D, S, Sigma, iter.max = 1000, nu = 0.0001, eps = 0.01, centered = FALSE, normalize = FALSE,weights){
  if(length(S)!=ncol(D)){
  ...
  lm.D = music.basic(Y, X, S, Sigma, iter.max = iter.max, nu = nu, eps = eps,weights=weights)
  return(lm.D)
}
